...
onIonViewDidEnter(async () => {
    ...
    fetchFacilitiesByCurrentStore(),
		store.dispatch("util/fetchStoreCarrierAndMethods", currentOrder.value.productStoreId),store.dispatch("util/fetchCarriersDetail"),
		store.dispatch("util/fetchSampleProducts")
	])
				...
				if(productIdsAlreadyAddedInList.includes(product.productId)) {
					...
					currentOrder.value.items.push({
						...
						quantity: quantityField
							? Number(uploadedItemsByIdValue[idValue][quantityField]) || 0
							: 0,
						isChecked: false,
						qoh: stock?.qoh || 0,
						atp: stock?.atp || 0,

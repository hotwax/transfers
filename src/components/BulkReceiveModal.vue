<template>
  <ion-header>
    <ion-toolbar>
      ...
      <ion-title>{{ translate("Bulk Receive") }}</ion-title>
    </ion-toolbar>
  </ion-header>
  ...
  <ion-content>
    <div v-if="!isProcessing && !isCompleted">
      <div class="ion-padding">
        <p>{{ translate("You are about to bulk receive") }} {{ itemsToProcess.length }} {{ translate("items.") }}</p>
        <p>
    ...
    <div v-else-if="isProcessing" class="ion-text-center">
      <div class="ion-padding">
        <ion-label>
          ...
          <p>{{ translate("Received") }} {{ completedItemsCount }} / {{ itemsToProcess.length }}</p>
        </ion-label>
      </div>
    ...
    <div v-else-if="isCompleted" class="ion-text-center">
      <div class="ion-padding">
        ...
        <p>{{ translate("Successfully received") }} <span data-testid="bulk-results-success-count">{{ successCount }}</span> {{ translate("items.") }}</p>
...
const pendingFulfillmentItemsCount = computed(() =>
  props.items.filter((item: any) => OrderActionValidator.isItemPendingFulfillment(props.order, item)).length
);

const itemsToProcess = computed(() =>
  props.items.filter((item: any) => !OrderActionValidator.isItemPendingFulfillment(props.order, item))
);
